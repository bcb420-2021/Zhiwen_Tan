...
### grouping
The original dataset does not indicate grouping, so I need to split all 4 groups manually
```{r}
dim(mets_data)
colnames(mets_data)
...
Since we need a dataframe as the final result, we need to make sure the dataframe is in correct formate.
...
Yes, there are about `r sum(is.na(mets_data$External_gene_name))` of gene could not be mapped to current HUGO symbols, there are `r length(mets_data$External_gene_name)` genes in total. After remove low counts genes, there are `r sum(is.na(mets_data_filtered$External_gene_name))` of genes could not mapped to current HUGO symbol, there are `r length(mets_data_filtered$External_gene_name)` genes in total.
